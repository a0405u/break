...
/*
    ...
    - Feature: Limit the maximum snoozes/skips
    - Feature: Detect non idle time without input
    - Feature: Account for the idle time on break
    - Feature: Warning sound
    - Feature: Time left on warning
    - Feature: Hard stop by solving puzzle / pressing long combination / writing
   a sentence
...
static void load_defaults(Config *config) {
  ...
  strcpy(config->break_message_text,
         "Rest your eyes. Stretch your legs. Breathe. Relax.");
  strcpy(config->break_hint_text, "^S - stop, ^Q - quit");

  strcpy(config->warning_message_text, "Please, take a break!");
  strcpy(config->warning_hint_text,
         "^Space - start, ^W - snooze, ^S - skip, ^Q - quit");

  strcpy(config->end_title_text, "Break has ended!");
  strcpy(config->end_message_text,
         "Work fruitfully. Concentrate on important. Don't get distracted.");
  ...
  config->respect_idle = true;
  config->min_idle = 1 * 60;
...
#define SET_COLOR(field)                                                       \
  if (strcmp(key, #field) == 0)                                                \
    ...
    SET_DURATION(min_idle);
    SET_BOOL(respect_idle);
...
void write_state(const char *state, uint ta, uint tb, uint tc) {
  ...
  snprintf(state_string, sizeof(state_string), "%s %i %i %i", state, ta, tb,
           tc);

  atomic_write(STATE_FILE, state_string);
}

char *get_font_string(const char *font_name, uint font_size,
                      const char *font_style, uint font_weight,
                      uint font_slant) {
...
int play_wav(const char *path, float volume) {
  ...
  if (memcmp(h.riff, "RIFF", 4) || memcmp(h.wave, "WAVE", 4) ||
      memcmp(h.fmt, "fmt ", 4) || h.audio_format != 1) // must be PCM
  {
    fclose(f);
    return -1;
  }
  ...
  ao_sample_format fmt = {.bits = h.bits_per_sample,
...
double pt_to_px(double pt, double dpi) { return pt * dpi / 72.0; }
...
static void load_xft_color(GlobalContext *gctx, const char *name,
                           XftColor *out) {
  if (!XftColorAllocName(gctx->display, gctx->visual, gctx->colormap, name,
                         out)) {
    die("Failed to load xft color!\n");
  }
}

static XftFont *load_xft_font(GlobalContext *gctx, const char *family, int size,
                              char *style, int weight, int slant) {
...
static void init(GlobalContext *gctx) {
  ...
  gctx->has_window = false;
  gctx->state = STATE_WAIT;
  ...
  gctx->idle_time = 0;
  gctx->total_idle_time = 0;
  gctx->next_state = 0;
  gctx->snoozes = 0;
  gctx->skips = 0;
  /*
  // Possible transparency
  if (XMatchVisualInfo(gctx->display, gctx->screen, 32, TrueColor,
  &gctx->vinfo))
  {
      gctx->depth = 32;
  ...
  */
  ...
  load_xft_color(gctx, gctx->config.background_font_color,
                 &gctx->background_font_color);
  ...
  /* ---- FONTS ---- */
  gctx->title_font = load_xft_font(
      gctx, gctx->config.font_name, gctx->config.title_font_size,
      gctx->config.title_font_style, gctx->config.title_font_weight,
      gctx->config.title_font_slant);

  gctx->message_font = load_xft_font(
      gctx, gctx->config.font_name, gctx->config.message_font_size,
      gctx->config.message_font_style, gctx->config.message_font_weight,
      gctx->config.message_font_slant);

  gctx->warning_font = gctx->message_font;

  gctx->hint_font =
      load_xft_font(gctx, gctx->config.font_name, gctx->config.hint_font_size,
                    gctx->config.hint_font_style, gctx->config.hint_font_weight,
                    gctx->config.hint_font_slant);

  gctx->time_font =
      load_xft_font(gctx, gctx->config.font_name, gctx->config.time_font_size,
                    gctx->config.time_font_style, gctx->config.time_font_weight,
                    gctx->config.time_font_slant);
}

static WindowContext spawn_window(GlobalContext *gctx, uint width, uint height,
  ...
  wctx.window =
      XCreateWindow(gctx->display, gctx->root, x, y, width, height, border,
                    gctx->depth, InputOutput, gctx->visual,
                    CWColormap | CWOverrideRedirect | CWBackPixel, &attrs);

  XMapWindow(gctx->display, wctx.window);
  XSync(gctx->display, False);

  Pixmap draw_buffer =
      XCreatePixmap(gctx->display, wctx.window, width, height, gctx->depth);
  XftDraw *draw_context =
      XftDrawCreate(gctx->display, draw_buffer, gctx->visual, gctx->colormap);
...
static void resize_window(GlobalContext *gctx, WindowContext *wctx, uint width,
                          uint height, int x, int y) {
...
static void draw_warning(GlobalContext *gctx, char *warning_text,
                         char *hint_text, uint time, WindowContext *wctx) {
  char text[256];
  ...
  XftTextExtentsUtf8(gctx->display, gctx->warning_font, (XftChar8 *)text,
                     strlen(text), &warning_extents);

  // Draw Warning Text

  int warning_text_x = (wctx->width - warning_extents.width) / 2;
  int warning_text_y = (wctx->height - warning_extents.height) / 2 +
                       warning_extents.height -
                       (warning_extents.height - warning_extents.y);
  XftDrawStringUtf8(wctx->draw_context, &gctx->font_color, gctx->warning_font,
                    warning_text_x, warning_text_y, (XftChar8 *)text,
                    strlen(text));
  ...
  if (hint_text && gctx->config.hints_enabled) {
    ...
    XftTextExtentsUtf8(gctx->display, gctx->hint_font, (XftChar8 *)hint_text,
                       strlen(hint_text), &hint_extents);

    int hint_text_x = (wctx->width - hint_extents.width) / 2;
    int hint_text_y = wctx->height - hint_extents.height;
    XftDrawStringUtf8(wctx->draw_context, &gctx->hint_font_color,
                      gctx->hint_font, hint_text_x, hint_text_y,
                      (XftChar8 *)hint_text, strlen(hint_text));
  }

  // Update display
  XCopyArea(gctx->display, wctx->draw_buffer, wctx->window,
            wctx->graphics_context, 0, 0, wctx->width, wctx->height, 0, 0);
  XFlush(gctx->display);
}

static void clear_window(GlobalContext *gctx, WindowContext *wctx,
                         XColor color) {
  XSetForeground(gctx->display, wctx->graphics_context, color.pixel);
  XFillRectangle(gctx->display, wctx->draw_buffer, wctx->graphics_context, 0, 0,
                 wctx->width, wctx->height);
}

static void draw_progress(GlobalContext *gctx, WindowContext *wctx,
                          double progress) {
  XSetForeground(gctx->display, wctx->graphics_context,
                 gctx->progress_color.pixel);
  ...
  int progress_y = 0;
  XFillRectangle(gctx->display, wctx->draw_buffer, wctx->graphics_context,
...
static void draw_message(GlobalContext *gctx, char *title_text,
                         char *message_text, char *hint_text, uint time,
                         WindowContext *wctx) {
  // Draw time left
  ...
  if (gctx->config.time_enabled) {
    ...
    XftTextExtentsUtf8(gctx->display, gctx->time_font, (XftChar8 *)time_text,
                       strlen(time_text), &time_extents);

    int time_text_x = (wctx->width - time_extents.xOff) / 2;
    int time_text_y = (wctx->height - time_extents.yOff) / 3;
    XftDrawStringUtf8(wctx->draw_context, &gctx->background_font_color,
                      gctx->time_font, time_text_x, time_text_y,
                      (XftChar8 *)time_text, strlen(time_text));
  }

  // Calculate text extents

  XGlyphInfo title_extents;
  XftTextExtentsUtf8(gctx->display, gctx->title_font, (XftChar8 *)title_text,
                     strlen(title_text), &title_extents);
  XGlyphInfo message_extents;
  XftTextExtentsUtf8(gctx->display, gctx->message_font,
                     (XftChar8 *)message_text, strlen(message_text),
                     &message_extents);
  ...
  int message_heigth = message_lines_count * message_extents.height +
                       (message_lines_count - 1) * message_extents.height;

  // Draw Title

  int title_text_x = (wctx->width - title_extents.width) / 2;
  int title_text_y =
      (wctx->height - title_extents.height - message_heigth - pixel_margin) /
          2 +
      title_extents.height - title_extents.y;

  XftDrawStringUtf8(wctx->draw_context, &gctx->font_color, gctx->title_font,
                    title_text_x, title_text_y, (XftChar8 *)title_text,
                    strlen(title_text));
  ...
  for (int i = 0; message_line; i++) {
    ...
    XftTextExtentsUtf8(gctx->display, gctx->message_font,
                       (XftChar8 *)message_line, strlen(message_line),
                       &message_line_extents);

    int message_line_x = (wctx->width - message_line_extents.width) / 2;
    int message_start_y =
        (wctx->height - title_extents.height - message_heigth - pixel_margin) /
            2 +
        title_extents.height + pixel_margin;
    int message_line_y = message_start_y + message_extents.height +
                         message_extents.height * i * 1.5 - message_extents.y;

    XftDrawStringUtf8(wctx->draw_context, &gctx->font_color, gctx->message_font,
                      message_line_x, message_line_y, (XftChar8 *)message_line,
                      strlen(message_line));
...
static void print_usage(const char *prog) {
  printf("Usage: %s [options]\n"
         ...
         "  -b, --break        Start break right away\n",
         prog);
}
...
GlobalState run_frame_event_loop(GlobalContext *gctx, FrameEventLoop *loop,
                                 void *userdata) {
  XEvent event;
  Timer timer = {0};
  timer_start(&timer);
...
static GlobalState process_wait(GlobalContext *gctx) {
  ...
  if (gctx->config.detect_idle) {
    ...
    for (uint t = 0; t < gctx->config.timer_duration; t++) {
      ...
      time_t idle_time = info->idle / 1000u; // Get current idle time
      if (idle_time < gctx->idle_time &&
          idle_time > gctx->config.min_idle)      // Idle reset
        gctx->total_idle_time += gctx->idle_time; // Save the latest idle time
      gctx->idle_time = idle_time;

      if (gctx->total_idle_time + gctx->idle_time >
          (unsigned long)gctx->config.idle_limit) {
        t = 0; // Reset timer
        gctx->total_idle_time = 0;
      }

      sleep(1);
      write_state("WAIT", t, gctx->config.timer_duration - t,
                  gctx->config.timer_duration);
    }
    XFree(info);
  } else {
    for (uint t = 0; t < gctx->config.timer_duration; t++) {
...
static void warning_on_frame(GlobalContext *gctx, double elapsed,
                             double duration, void *ud) {
  (void)ud;
  ...
  draw_warning(gctx, gctx->config.warning_message_text,
               gctx->config.warning_hint_text, (int)time_left, &gctx->wctx);
...
static GlobalState warning_on_event(GlobalContext *gctx, XEvent *event,
                                    void *ud) {
  (void)ud;
  ...
  if (event->type == ButtonPress) {
    ...
    case XK_w: // Snooze
      if (ctrl && gctx->config.snooze_enabled &&
          (gctx->snoozes < gctx->config.max_snoozes))
        return STATE_SNOOZE;
      break;
    case XK_s: // Skip
      if (ctrl && gctx->config.skip_enabled &&
          (gctx->skips < gctx->config.max_skips))
        return STATE_SKIP;
      break;
    case XK_q: // Quit
      if (ctrl)
...
static GlobalState warning_on_exit(GlobalContext *gctx, GlobalState state,
                                   void *ud) {
  (void)gctx, (void)ud;
...
static GlobalState process_warning(GlobalContext *gctx) {
  ...
  gctx->wctx =
      spawn_window(gctx, warning_width, warning_height, warning_x, warning_y,
                   gctx->config.border_width, &gctx->background_color, true);
  gctx->has_window = true;
  ...
  // XGrabKeyboard(gctx->display, gctx->wctx.window, True, GrabModeAsync,
  // GrabModeAsync, CurrentTime);
  XSelectInput(gctx->display, gctx->wctx.window,
               KeyPressMask | ButtonPressMask);

  FrameEventLoop loop = {.on_frame = warning_on_frame,
...
static void break_on_frame(GlobalContext *gctx, double elapsed, double duration, void *ud) {
  ...
  clear_window(gctx, &gctx->wctx, gctx->background_color);
  draw_progress(gctx, &gctx->wctx, progress);
  draw_message(gctx, gctx->config.break_title_text,
               gctx->config.break_message_text, gctx->config.break_hint_text,
               time_left, &gctx->wctx);
...
static GlobalState break_on_event(GlobalContext *gctx, XEvent *event,
                                  void *ud) {
  (void)ud;
...
static GlobalState break_on_exit(GlobalContext *gctx, GlobalState state,
                                 void *ud) {
  (void)ud;
...
static GlobalState process_break(GlobalContext *gctx) {
  ...
  if (gctx->has_window)
    resize_window(gctx, &gctx->wctx, gctx->screen_width, gctx->screen_height, 0,
                  0);
  else {
    // gctx->prev_window = gctx->wctx.window;
    gctx->wctx = spawn_window(gctx, gctx->screen_width, gctx->screen_height, 0,
                              0, 0, &gctx->background_color, true);
  }
  gctx->has_window = true;
  set_input_focus(gctx, gctx->wctx.window);
  XRaiseWindow(gctx->display, gctx->wctx.window);
  ...
  if (gctx->config.block_input) {
    ...
    XGrabPointer(gctx->display, gctx->wctx.window, True,
  ...
  draw_message(gctx, gctx->config.break_title_text,
               gctx->config.break_message_text, gctx->config.break_hint_text,
               gctx->config.break_duration, &gctx->wctx);
  ...
  time_t idle_time =
      gctx->config.respect_idle ? gctx->idle_time + gctx->total_idle_time : 0;

  FrameEventLoop loop = {.on_frame = break_on_frame,
                         .on_event = break_on_event,
                         .on_exit = break_on_exit,
                         .duration = gctx->config.break_duration - idle_time};
...
static GlobalState process_end(GlobalContext *gctx) {
  ...
  draw_message(gctx, gctx->config.end_title_text, gctx->config.end_message_text,
               gctx->config.end_hint_text, 0, &gctx->wctx);
...
static GlobalState process_snooze(GlobalContext *gctx) {
  ...
  // XSetInputFocus(gctx->display, gctx->last_focus, RevertToParent,
  // CurrentTime);
  XFlush(gctx->display);
  ...
  for (uint t = 0; t < gctx->config.snooze_duration; t++) {
    sleep(1);
    write_state("SNOOZE", t, gctx->config.snooze_duration - t,
                gctx->config.snooze_duration);
  }
...
static GlobalState process_skip(GlobalContext *gctx) {
...
int main(int argc, char **argv) {
  ...
  while (gctx.state != STATE_EXIT) {
    switch (gctx.state) {
    case STATE_WAIT:
      gctx.state = process_wait(&gctx);
      break;
    case STATE_WARNING:
      gctx.state = process_warning(&gctx);
      break;
    case STATE_SNOOZE:
      gctx.state = process_snooze(&gctx);
      break;
    case STATE_BREAK:
      gctx.state = process_break(&gctx);
      break;
    case STATE_SKIP:
      gctx.state = process_skip(&gctx);
      break;
    case STATE_RESTART:
      gctx.state = process_restart(&gctx);
      break;
    case STATE_END:
      gctx.state = process_end(&gctx);
      break;
    default:
      break;
    }
